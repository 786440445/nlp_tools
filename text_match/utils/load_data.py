...
from text_match.utils.data_utils import shuffle, pad_sequences

vocab_path = os.path.join(home_dir, 'text_match/data/vocab.txt')
...
class args:
    ...
    max_char_len = 50
    max_word_len = 50
    corpus_path = 'text_match/data/lcqmc'
...
def load_all_data(path, data_size=None):
    model = Word2Vec.load('../output/word2vec/dialog_word2vec.model')
    df = pd.read_csv(path)
    p = df['sentence1'].values[0:data_size]
    h = df['sentence2'].values[0:data_size]
    label = df['label'].values[0:data_size]
